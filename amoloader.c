...
struct amoloader_data *load_amo(const char *path)
{
     ...
     * If its a valid amo keyword read the arguments and put them into the right position in the data struct */
    while(fscanf(file, "%s", cmd_buf) != EOF) {
        ...
        /* vw <weight_1> <weight_2> <weight_3> <weight_4> */
            ...
            weight_count++;
        /* f <pos>/<normal>/<uv>/<joint>/<weight> <pos>/<normal>/<uv>/<joint>/<weight> <pos>/<normal>/<uv>/<joint>/<weight> 
         * Currently, the library expects <pos>/<normal>/<uv>/<joint>/<weight> to be the same value, since editamo exports it like that.
    ...
    /* This is the part, where the logic to order the buffer based on f should come in.
